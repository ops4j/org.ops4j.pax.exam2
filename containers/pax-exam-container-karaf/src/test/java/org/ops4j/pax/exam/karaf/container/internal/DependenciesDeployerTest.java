...
public class DependenciesDeployerTest {
    ...
    @Test
    public void testDependencyFeatureWithDependentFeatures() {
    	
    	// GIVEN
    	KarafFeaturesOption option = KarafDistributionOption.features("mvn:mygroup/myArtifactId/1.0/xml/features", "myFeature");
    	
    	ExamSystem subsystem = Mockito.mock(ExamSystem.class);
    	Mockito.when(subsystem.getOptions(Mockito.eq(ProvisionOption.class))).thenReturn(new ProvisionOption[0]);
    	Mockito.when(subsystem.getOptions(Mockito.eq(KarafFeaturesOption.class))).thenReturn(new KarafFeaturesOption[] {option});
		File karafBase = new File(System.getProperty("java.io.tmpdir"));
		File karafHome = karafBase;
		DependenciesDeployer dependenciesDeployer = new DependenciesDeployer(subsystem, karafBase, karafHome);
		...
		Assert.assertArrayEquals(new String[] {"test-dependencies"}, dependenciesFeature.getFeatures());
		...
		try (Scanner scanner = new Scanner(outputFeaturesFile)) {
			...
			xmlOutputString = "FILE NOT FOUND: " + outputFeaturesFile.toString();
		}
