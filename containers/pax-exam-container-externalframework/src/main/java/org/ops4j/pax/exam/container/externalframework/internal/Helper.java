...
/**
 * This class is extract form test karaf tool.
 * We cannot use it because it depends form the default container.
 */
public final class Helper {
    ...
    public static Option[] getDefaultConfigOptions(Properties sysProps, URL configProperties, SystemPropertyOption... sysOptions) {
     ...
     // add test-specific system properties
        for (SystemPropertyOption sysOption : sysOptions) {
     	   configProps.setProperty(sysOption.getKey(), sysOption.getValue());
        }
        ...
        for (Enumeration<?> e = configProps.propertyNames(); e.hasMoreElements();) {
            ...
            if ("org.osgi.framework.system.packages".equals(name)) {
            	...
            	if (extra != null && extra.length() > 0) {
                	options.add(systemProperty(name).value(value + "," + extra));
                } else {
                	options.add(systemProperty(name).value(value));
                }
            } else if ("org.osgi.framework.bootdelegation".equals(name)) {
                options.add(bootDelegationPackages(value));
            } else if ("org.osgi.framework.startlevel.beginning".equals(name)) {
     ...
     * @return default pax-exam options for karaf osgi framework
     */
    public static Option[] getDefaultConfigOptions(String karafHome, String config, SystemPropertyOption... sysOptions) {
        return Helper.getDefaultConfigOptions(Helper.getDefaultSystemOptions(karafHome),
        					getResource(config),
                                       sysOptions);
    }
     ...
     * @return an array of pax-exam options for provisioning karaf system bundles
     */
    public static Option[] getDefaultProvisioningOptions(String karafHome, String resource) {
        return Helper.getDefaultProvisioningOptions(Helper.getDefaultSystemOptions(karafHome),
     ...
     * @return an array of pax-exam options
     */
    public static Option[] getDefaultOptions(String karafHome, String config, String startup, SystemPropertyOption... sysOptions) {
        return combine(getDefaultConfigOptions(karafHome, config, sysOptions), getDefaultProvisioningOptions(startup, karafHome));
    }
}
