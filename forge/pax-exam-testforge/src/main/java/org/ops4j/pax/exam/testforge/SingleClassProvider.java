 ...
 * 
 * Inspired by Gogos "which bundleid classname" command. It makes sure that a class is (if
 * loadable!) loaded by the same source. Multiple sources for the same class is not bad per-se, but
 * some classes (like LoggerFactory) and stuff should really be loaded from one single source.
 * Otherwise its source of trouble. This is what this test checks.
 * 
 */
public class SingleClassProvider {
     ...
     * @param clazzName
     *            Class to load from all bundles and check sources.
     * @param optionalSkipSystemBundle
     *            optional: set to true if System Bundle (ID=0) should be skipped.
     * @throws Exception
     */
    public void probe(BundleContext ctx, String clazzName, Boolean optionalSkipSystemBundle)
        ...
        for (Bundle b : ctx.getBundles()) {
            ...
            if (commonSource == UNCHECKED && loaded != CLAZZ_NOT_FOUND) {
                ...
                // skip that. Not every bundle needs to be able to load that clazz of cause. This is
                // not our test.
            }
            else {
                if (commonSource != loaded) {
                    throw new TestContainerException("Test failed because class " + clazzName
                        + " has been loaded from at least two different sources: "
                        + map(commonSource, sourceName) + " vs. "
                        + map(loaded, b.getSymbolicName()) + ".");
                }
            }
        }
    }
